...
export function TourCard({ tour, priority = false }: TourCardProps) {
  ...
  return (
    <article className="group mx-auto flex h-full w-full max-w-sm overflow-hidden rounded-lg border border-stone-200/90 bg-white shadow-[0_18px_55px_rgba(32,25,19,0.08)] ring-1 ring-white/70 transition duration-300 hover:-translate-y-1 hover:border-[#d8b778] hover:shadow-[0_28px_70px_rgba(32,25,19,0.16)] lg:max-w-none">
      <div className="flex min-h-full w-full flex-col">
        <div className="relative aspect-[4/3] overflow-hidden bg-[#f1dfbf] sm:aspect-[16/11]">
          <Image
            src={tour.cardImage}
          ...
          />
          <div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(19,32,40,0.12),rgba(19,32,40,0.05)_38%,rgba(19,32,40,0.78))]" />
        ...
        <div className="flex flex-1 flex-col p-4 sm:p-5">
          <div className="grid gap-3">
            <div className="flex flex-wrap gap-2">
              {tour.route.slice(0, 4).map((stop) => (
                <span key={stop} className="rounded-full bg-[#fbf7ef] px-2.5 py-1 text-[11px] font-black text-[#6d6256] ring-1 ring-stone-200">
                  {stop}
                </span>
              ))}
            </div>
          ...
          <div className="mt-auto pt-4">
            <div className="grid grid-cols-2 overflow-hidden rounded-md border border-stone-200 bg-white">
              ...
              <div className="p-3">
                <p className="text-[10px] font-black uppercase tracking-[0.14em] text-[#6d6256]">From</p>
                <p className="mt-1 text-lg font-black leading-none text-[#b5532f]">&euro;{tour.startingPrice}</p>
              </div>
            </div>

            <Link
              href={`/tours/${tour.slug}`}
              className="mt-3 inline-flex w-full items-center justify-center rounded-full bg-[#132028] px-5 py-3 text-sm font-black text-white shadow-[0_14px_30px_rgba(19,32,40,0.18)] transition hover:bg-[#b5532f] focus:outline-none focus:ring-2 focus:ring-[#b5532f] focus:ring-offset-2"
            >
              View tour
